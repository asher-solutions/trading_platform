// assets/javascript/components/BuilderPanel.jsx
...
const BuilderPanel = ({ isFullScreen, setIsFullScreen }) => {
  ...
  const getAnchorPosition = (component, anchor: 'top' | 'right' | 'bottom' | 'left'): { x: number; y: number } => {
    const width = 100;
    const height = 100;
    switch (anchor) {
      case 'top':
  ...
  const getClosestAnchor = (component, point: { x: number; y: number }): 'top' | 'right' | 'bottom' | 'left' => {
